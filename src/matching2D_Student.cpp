...
void detKeypointsShiTomasi(vector<cv::KeyPoint> &keypoints, cv::Mat &img, double& t, bool bVis)
{
    ...
    for (size_t j = 0; j < dst_norm.rows; j++) {
        for (size_t i = 0; i < dst_norm.cols; i++) {
            ...
            if (response > minResponse) {
                ...
                if (!bOverlap) {
                    // only add new key point of no overlap has been found in previous NMS
                    keypoints.push_back(newKeyPoint); // store new keypoint in dynamic list
                }
            }
        }
    }
    t = 1000 * ((double)cv::getTickCount() - t) / cv::getTickFrequency() / 1.0;
